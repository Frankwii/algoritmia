...
def busqueda_ultima_casilla(tablero, N, M, K, n, ltransitables):
    ...
    Para comprobar si existe camino al quitar la casilla m-esima, debemos quitar las m
    primeras casillas. Si existe camino, es que la casilla buscada tiene posicion >m,
    y por tanto podemos descartar la primera mitad de la lista y dejar quitadas esas casillas.
    Si no existe, es que la casilla buscada tiene posicion <m. Luego, cuando nos
    situemos en el centro del nuevo intervalo, debemos volver a colocar las casillas que
    van despues de el.
    """
...
def buscar_camino(tablero, N, M, K, K2):  # K2=K**2; se pasa por parametro para no tener que calcularlo cada vez
    ...
    punto (0,0) hasta el punto (N,M). Para hacerlo, se centra en una casilla transitable
    - inicalmente, (0,0) - y mira todas las casillas transitables a las que puede acceder
    desde alli. Las anade a una pila para posteriormente mirar sus vecinos, y las
    ...
    while long_pila != 0:
        ...
        """Como solo nos interesan las casillas transitables a distancia <= K de (x,y),
...
while len(entrada) == 4:
    ...
    # Marcamos las casillas transitables
    ltransitables = [(0, 0)] * n
    for i in range(n):
        entrada = sys.stdin.readline().split(" ")
        x = int(entrada[0])
        ...
        if x < 0 or x > N or y < 0 or y > M:
            print("ERROR: casilla fuera del tablero")
            break
        if tablero[x][y] == 1:
            print("ERROR: transitable dos veces")
            break
